...
class SpmDetails extends Component {
    constructor({match}) {
        super();
        this.state = {
            uid:match.params.id,
            name:'',phone:'',type:'',email:'',status:'',date:'',address:'',token:'',image:'',image2:'',
            loading:true,payment_type:'',account:'',records: [],records2:[]
        }
        ...
        this.columns2 = [
            ...
            {key: "t_sold", text: "Total Sold Amount"},
            {key: "c_amount", text: "Total Collection amount"},
            {key: "c_charge", text: "Total Collection Delivery charge"},
            {key: "date", text: "Created Date",
                cell: record => {
                    return(
                        <Fragment>
    ...
    componentDidMount() {
        ...
        if(token){
            ...
            Axios.post(AppUrl.SpmOrders,myFormData,config)
                .then(res=>{
            ...
            Axios.post(AppUrl.SpmPaymentHistory,myFormData,config)
                .then(res=>{
                    this.setState({records2:res.data,loading:false})
                })
                .catch(error=>{
                    console.log(error)
                })
            ...
            <>
                ...
                <div className="content-wrapper">
                    <div className="row m-2  pt-3">
                        ...
                        <div className="col-md-12 col-12 bg-white py-3  table-responsive-sm mb-5 mt-2">
                            <h5 className="body-title pl-5">Payment Management {this.state.name} ({this.state.records2.length }) </h5>
                            <ReactDatatable
                                config={this.config}
                                records={this.state.records2}
