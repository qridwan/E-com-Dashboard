...
class EditBlog extends Component {
    ...
    componentDidMount() {
        ...
        if(token){
            ...
            Axios.post(AppUrl.oneBlog,myFormData,config)
                .then(res=>{
                    console.log("Userdata "+res.data)
                    this.setState({title:res.data[0]['title'],image:res.data[0]['image'],des:res.data[0]['body'],loading:false})
                })
                .catch(error=>{
                    localStorage.clear()
                    console.log(error)
    ...
    FormSubmit(event){
        ...
        Axios.post(AppUrl.updateBlog, formData)
            .then(res=>{
                if(res.data.success){
