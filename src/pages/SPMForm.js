...
class SPMForm extends Component {
    constructor() {
        super();
        this.state = {
            data: [],
                        ...
                        <Fragment>
                            <div className="row">
                                <div className="col-md-12 col-sm-12 col-lg-12">
                                    <Link to={"/spmform/view/"+record.id}  className="btn btn-primary btn-sm col-md-4 col-lg-4 col-sm-6 m-1  ">
    ...
    componentDidMount() {
        ...
        if(token){
            ...
            Axios.post(AppUrl.spmformList,myFormData,config)
                .then(res=>{
    ...
    onDel(e){
        const spmform_id = e.target.name
        const token = this.state.token
        const formData = new FormData()
        formData.append('id',spmform_id)
        formData.append('token',token)
        Axios.post(AppUrl.deleteSpmform,formData)
            .then(res=>{
    ...
    render() {
        ...
        return (
            <Fragment>
                <Header title="Service Point Form List"/>
                <div className="content-wrapper">

                    {this.state.loading==true ? <Loading/> :
                        <div className="row m-3 bg-white pt-3">
                            <p className="body-title">SPM Form Review</p>
